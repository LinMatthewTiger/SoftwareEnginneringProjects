...
import br.ol.pacman.infra.Image;
...
public class BitmapFontRenderer implements Serializable {
    ...
    private void loadFont(String filename, Integer cols, Integer rows) {
        try {
            bitmapFontImage = (Image) ImageIO.read(getClass().getResourceAsStream(filename));
    ...
    private void loadFont(Image image, Integer cols, Integer rows) {
        int lettersCount = cols * rows; 
        bitmapFontImage = image;
        letters = new Image[lettersCount];
        letterWidth = ((BufferedImage)bitmapFontImage).getWidth() / cols;
        letterHeight = ((BufferedImage)bitmapFontImage).getHeight() / rows;

        for (int y=0; y<rows; y++) {
            for (int x=0; x<cols; x++) {
                letters[y * cols + x] = new Image(letterWidth, letterHeight, BufferedImage.TYPE_INT_ARGB);
