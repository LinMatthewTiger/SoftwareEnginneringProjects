...
public class phase3 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nBest SSE: " + bestSSE + "      Best Run: " + bestRun + "      Best Cen: " + bestCen);
	...
	public static void SSE() {
		...
		System.out.print("Iteration " + printSSE + ": obj = " + SSE + "\n");
	...
	public static void SWindex() {
		...
		for (int k = 0; k < numCen; k++) {
			...
			for (int m = 0; m < allClosestPoints.get((int) bestRun).get(k).size(); m++) {
				...
				inDist = inDist / (allClosestPoints.get((int) bestRun).get(k).size() - 1);
		...
		System.out.println("Average SW " + avgsi);
	}
	...
	public static void DBIndex() {
		// 1. Find the standard deviation for each cluster.
		// 2. Find the distance between the cluster centers.
		// 3. Add the stds divided by the distance between the two cluster centers.
...
//		}
		...
		for (int k = 0; k < numCen; k++) {
			...
			sd.add(sdval / allClosestPoints.get((int) bestRun).get(k).size());
		}
				...
				if (m != l) {
					dbval = (sd.get(l) + sd.get(m))
							/ (calculateDistance((ArrayList<Double>) allCents.get((int) bestRun).get(l),
									allCents.get((int) bestRun).get(m)));
				}
		...
		for (int i = 0; i < finaldbvals.length; i++) {
			finaldb += finaldbvals[i];
		}

		finaldb /= finaldbvals.length;

		System.out.println("DB " + finaldb);
